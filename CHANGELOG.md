...
- 会場をUUIDに紐づくプルダウンと追加モーダルへ変更。以前選んだ会場名を別会場の名前で上書きしない。
- 撮影者／カードNoを、ローカル下書きから一括確定するボタン選択モーダルへ変更。登録カードの撮影者連動、先頭ゼロの保持、新規登録、取消時の無変更に対応。
- 保存済みプロジェクトの保存先変更は、その保存先だけを保持する。シーン／撮影情報等の未保存変更は勝手に保存せず、新規プロジェクトには保存が必要と案内する。
- 素材の種類を常時見える排他ボタンへ変更。狭いペインでは折り返し、取り込み／評価の両画面で利用する。
- シーンを「1日目〜4日目／その他」のボタンで切り替える。追加の日数も保持し、非表示シーンへの誤割当を防止。LAN更新で同じシーンUUIDが残れば選択日を維持する。

### Safety

- 起動時から接続中のカードを含め、安全な物理ID／挿入世代／永続隔離照合を通過したSDが1枚だけの場合に取り込み元を自動選択する。複数枚、別画面、処理中、モーダル中は自動選択しない。手動選択済みのソースを置き換えず、コピーと初期化は自動開始しない。
- 自動認定できないUSBリーダー等を通常フォルダ扱いへ自動降格しない。手動選択を案内し、既存のカード隔離・初期化禁止を維持する。
- プロジェクト選択の先行ID更新を廃止。ロード失敗／認可拒否で画面と対象IDが食い違わないようにし、不一致IDのプロジェクト削除を拒否する。

## [0.2.0-alpha.6] - 2026-09-13
